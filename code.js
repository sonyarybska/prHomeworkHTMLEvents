...
let a;
let box1=document.querySelector('#status');
let box2=document.querySelector('#age');
let box3=document.querySelector('#city ');
let divka=document.querySelector('#divka');
document.body.appendChild(divka);
let myArray=JSON.parse(JSON.stringify(usersWithAddress));

box1.onclick=()=>{
    if (box1.checked) myArray=myArray.filter(value=>value.isMarried===false);
    let string=JSON.stringify(myArray);
    divka.innerHTML=string;

}
box2.onclick=()=> {
    if (box2.checked) myArray=myArray.filter(value => value.age >= 29);
...
box3.onclick=()=> {
    if (box3.checked) myArray =myArray.filter(value => value.address.city === 'Kyiv');
    let string=JSON.stringify(myArray);
    divka.innerHTML=string;
}
*/
